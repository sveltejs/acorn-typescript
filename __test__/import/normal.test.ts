...
describe('normal syntax', () => {
	...
	it('import namespace type specifiers', function () {
		const node = parseSource(
			generateSource([`import test, { name, type age, school as school1 } from './index.js'`])
		);
	...
	it('import type specifier with as', function () {
		const node = parseSource(generateSource([`import test, { type as age } from './index.js'`]));

		equalNode(node, NormalImportSnapshot.ImportTypeSpecifierWithAs);
	});

	it('import type specifier with as as', function () {
		const node = parseSource(generateSource([`import test, { type as as } from './index.js'`]));
	...
	it('import type specifier with as as', function () {
		const node = parseSource(
			generateSource([`import { type as as somethings } from './index.js'`])
		);
	...
	it('issue 45', function () {
		const node = parseSource(generateSource([`import assert from './index.js'`]));
