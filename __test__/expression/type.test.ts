...
import { equalNode, generateSource, parseDtsSource, parseSource } from '../utils';
...
describe('expression type test', () => {
	...
	it('normal type', () => {
		const node = parseSource(generateSource([`type School = 'Tsinghua' | 'Peking' | 'GDUT'`]));
	...
	it('enum', () => {
		const node = parseSource(
			generateSource([`enum Test {`, `  Start = 'start',`, `  End = 'end'`, `}`])
		);
