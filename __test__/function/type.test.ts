...
describe('function type test', () => {
  it('no parameter with void', () => {
    const node = parseSource(
      generateSource([`function test(): void {`, `  console.log(123)`, `}`])
    );
  ...
  it('no parameter with never', () => {
    const node = parseSource(
      generateSource([`function test(): never {`, `  throw new Error('123')`, `}`])
    );
  ...
  it('no parameter with string', () => {
    const node = parseSource(generateSource([`function test(): string {`, `  return '123'`, `}`]));

    equalNode(node, FunctionTypeSnapshot.NoParameterWithString);
  });

  it('no parameter with number', () => {
    const node = parseSource(generateSource([`function test(): number {`, `  return 123`, `}`]));
  ...
  it('no parameter with undefined', () => {
    const node = parseSource(
      generateSource([`function test(): undefined {`, `  return undefined`, `}`])
    );
  ...
  it('no parameter with boolean', () => {
    const node = parseSource(generateSource([`function test(): boolean {`, `  return true`, `}`]));
  ...
  it('no parameter with bigint', () => {
    const node = parseSource(
      generateSource([`function test(): bigint {`, `  return BigInt('123123')`, `}`])
    );
  ...
  it('no parameter with object', () => {
    const node = parseSource(
      generateSource([`function test(): object {`, `  return { a: 1 }`, `}`])
    );
  ...
  it('no parameter with symbol', () => {
    const node = parseSource(
      generateSource([`function test(): symbol {`, `  return Symbol('123')`, `}`])
    );
  ...
  it('no parameter with unknown', () => {
    const node = parseSource(generateSource([`function test(): unknown {`, `  return 123`, `}`]));

    equalNode(node, FunctionTypeSnapshot.NoParameterWithUnknown);
  });

  it('no parameter with any', () => {
    const node = parseSource(generateSource([`function test(): any {`, `  return 123`, `}`]));
  ...
  it('no parameter with union', () => {
    const node = parseSource(
      generateSource([`function test(): string | number {`, `  return 123`, `}`])
    );
  ...
  it('one parameter with void', () => {
    const node = parseSource(
      generateSource([`function test(name: string): void {`, `  console.log(name)`, `}`])
    );
  ...
  it('async arrow function with one param', () => {
    const node = parseSource(generateSource([`a = async x => {}`]));
  ...
  it('declare function comma after rest element', () => {
    const node = parseSource(
      generateSource([`declare function test(a: number | string, ...b,): number | string;`])
    );
  ...
  it('arrow function with optional param', () => {
    const node = parseSource(generateSource([`const test = (name: string, age?: number) => 42`]));
