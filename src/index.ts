...
function tsPlugin(options?: {
	...
	return function (Parser: typeof AcornParseClass) {
		...
		class TypeScriptParser extends Parser {
			...
			tsTryParseDeclare(nany: any): any | undefined | null {
				...
				return this.tsInAmbientContext(() => {
					...
					if (tokenIsIdentifier(starttype)) {
						return this.tsParseDeclaration(nany, this.value, /* next */ true);
					}
				});
			}
			...
			tsParseTypeAnnotation(eatColon = true, t: any = this.startNode()): any {
				this.tsInType(() => {
			...
			tsTryParseAndCatch<T extends any | undefined | null>(f: () => T): T | undefined | null {
			...
			tsParseList<T extends Node>(kind: ParsingContext, parseElement: () => T): T[] {
				const result: T[] = [];
			...
			tsParseModuleOrNamespaceDeclaration(node: any, nested: boolean = false): any {
				node.id = this.parseIdent();
			...
			tsTryParseExportDeclaration(): any | undefined | null {
				return this.tsParseDeclaration(this.startNode(), this.value, /* next */ true);
			}
			...
			parseExport(node: any, exports: any): any {
				...
				if (this.ts_eatWithState(tt._import, 2, enterHead)) {
					...
					const exportEqualsNode = this.tsParseImportEqualsDeclaration(node, /* isExport */ true);
			...
			parseExportDeclaration(node: any): any | undefined | null {
				...
				const declaration =
					(isIdentifier && this.tsTryParseExportDeclaration()) || this.parseStatement(null);
			...
			parseAssignableListItem(allowModifiers: boolean | undefined | null) {
				const decorators = [];
